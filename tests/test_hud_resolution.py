...
def hud(tmp_path, monkeypatch):
    ...
    cfg.update(memory_db_path=str(tmp_path / "test.db"), log_path=str(tmp_path / "test.log"),
               system_prompt_path=str(repo / "config/system_prompt.txt"),
               planner_schema_path=str(repo / "config/planner_schema.json"),
               voice={"enabled": False}, hardware={"drivers": []},
               anticipation={"enabled": False})
    monkeypatch.chdir(tmp_path)
    monkeypatch.setattr(server, "_load_config", lambda: cfg)
    with TestClient(server.app) as client:
        feedback = CorrectionFeedbackStore(server._state["mem"],
                                            enabled=lambda: server._state["episodes"].enabled)
        server._state["resolver"] = CommandResolver(
            [IntuitionCommandProvider(), StaticCommandProvider(["git", "python", "echo"], case_sensitive=False)],
...
def submit(ws, snapshot, index=0, **overrides):
    message = {"type": "input", "text": snapshot["original"],
               "selected_text": snapshot["original"] if index is None else snapshot["candidates"][index]["text"],
               "token": snapshot["token"], "revision": snapshot["revision"],
...
def test_hud_displays_raw_preserving_correction_and_safe_mode_blocks_execution(hud):
    ...
    with client.websocket_connect("/ws") as ws:
        ...
        assert shown["candidates"][0]["text"] == '  python\ttrain.py --key "secret-value"  '
        assert shown["candidates"][0]["span"] == [2, 8]
        submit(ws, shown)
        assert "Safe Mode" in receive(ws, "reply")["text"]
        feedback = server._state["mem"].query("SELECT original_token,selected_token,accepted,outcome,candidates_json FROM command_corrections")
...
def test_hud_binds_confirmed_execution_to_displayed_arguments_once(hud, monkeypatch):
    set_safe_mode(False)
    calls = []
    monkeypatch.setitem(capabilities._caps, "run_command", replace(capabilities.get("run_command"), fn=
                        lambda **kwargs: calls.append(kwargs) or {"stdout": "showcase: reviewed command", "returncode": 0}))
    client, _ = hud
    with client.websocket_connect("/ws") as ws:
        receive(ws, "status")
        ...
        ws.send_json({"type": "confirm", "token": confirmation["token"], "granted": True,
...
def test_edit_invalidates_both_displayed_correction_and_parked_approval(hud, monkeypatch):
    set_safe_mode(False)
    calls = []
    monkeypatch.setitem(capabilities._caps, "run_command", replace(capabilities.get("run_command"), fn=lambda **kwargs: calls.append(kwargs) or {}))
    client, _ = hud
    with client.websocket_connect("/ws") as ws:
        receive(ws, "status")
...
def test_other_connection_cannot_submit_a_resolution_or_confirm_its_action(hud):
    ...
    with client.websocket_connect("/ws") as first, client.websocket_connect("/ws") as other:
        ...
        other.send_json({"type": "confirm", "token": confirmation["token"], "granted": True})
        assert "another connection" in receive(other, "error")["text"]
        first.send_json({"type": "confirm", "token": confirmation["token"], "granted": False})
...
def test_manual_edit_feedback_and_forget_clear_every_connection(hud):
    ...
    with client.websocket_connect("/ws") as ws, client.websocket_connect("/ws") as other:
        ...
        rows = server._state["mem"].query("SELECT manual_token,accepted FROM command_corrections WHERE original_token='gti'")
        assert rows == [("git", None)]
        ws.send_json({"type": "input", "text": "/forget"})
        receive(ws, "reply")
        receive(other, "input_invalidated")
        assert server._state["mem"].query("SELECT COUNT(*) FROM command_corrections")[0][0] == 0
...
def test_logging_disable_drops_pending_views_and_stops_learning(hud):
    ...
    with client.websocket_connect("/ws") as ws:
        ...
        receive(ws, "reply")
        before = server._state["mem"].query("SELECT COUNT(*) FROM command_corrections")[0][0]
        submit(ws, stale)
        receive(ws, "error")
        fresh = preview(ws, "/hlep", revision=2)
        submit(ws, fresh)
        receive(ws, "reply")
        assert server._state["mem"].query("SELECT COUNT(*) FROM command_corrections")[0][0] == before
...
def test_advertised_hud_commands_have_handlers(hud):
    ...
    with client.websocket_connect("/ws") as ws:
        receive(ws, "status")
        for text in ("/help", "/config", '/write example.txt "hello review"', "/hw schema missing",
...
def test_voice_transcript_is_draft_and_never_executes(hud):
    ...
    class FakeVoice:
        def is_recording(self):
            return False
        def start_recording_vad(self, on_silence, on_complete):
...
def test_existing_os_intentions_remain_exact_and_reach_their_capability(hud, monkeypatch, text, action):
    calls = []
    monkeypatch.setitem(capabilities._caps, action, replace(capabilities.get(action),
...
def test_supported_shell_failure_is_feedback_error_not_rejected_interpretation(hud, monkeypatch):
    set_safe_mode(False)
    monkeypatch.setitem(capabilities._caps, "run_command", replace(capabilities.get("run_command"),
                        fn=lambda **kwargs: {"stdout": "", "stderr": "no repository", "returncode": 128}))
    client, _ = hud
    with client.websocket_connect("/ws") as ws:
        receive(ws, "status")
        ...
        receive(ws, "reply")
        receive(ws, "status")
        rows = server._state["mem"].query("SELECT accepted,outcome FROM command_corrections")
        assert rows == [(1, "error")]
...
def test_shutdown_cannot_restore_forgotten_predictor(tmp_path, monkeypatch):
    ...
    cfg.update(memory_db_path=str(tmp_path / "test.db"), log_path=str(tmp_path / "test.log"),
               system_prompt_path=str(repo / "config/system_prompt.txt"),
               planner_schema_path=str(repo / "config/planner_schema.json"),
               voice={"enabled": False}, hardware={"drivers": []}, anticipation={"enabled": False})
    monkeypatch.setattr(server, "_load_config", lambda: cfg)
    monkeypatch.chdir(tmp_path)
    with TestClient(server.app) as client:
