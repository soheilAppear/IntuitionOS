"""Existing local OS intent grammar, shared by command resolution and HUD."""
import re


def is_app_command(text):
    """Reserve existing app phrases without searching arbitrary shell arguments."""
    stripped = text.strip()
    if stripped in {"ls", "tree"} or re.match(r"^(?:remind\s+me|read\s+file)\s+", stripped, re.I):
        return True
    # The OS matcher historically searches within a natural-language sentence.
    # Restrict its prefix here so `git commit -m battery` stays a Git command.
    prefix = stripped.lower().split(maxsplit=1)[0] if stripped else ""
    natural_prefixes = {
        "open", "launch", "start", "run", "execute", "set", "put", "change", "turn", "make",
        "adjust", "volume", "sound", "mute", "no", "silent", "silence", "raise", "increase",
        "lower", "decrease", "what", "what's", "check", "get", "show", "brightness", "dim",
        "battery", "charge", "power", "how", "am", "which", "network", "ip", "wifi", "wi-fi",
        "enable", "disable", "sleep", "hibernate", "suspend", "lock", "shutdown", "shut",
        "restart", "reboot", "cancel", "screenshot", "screen", "snap", "take", "system", "sys",
        "pc", "hardware", "computer", "machine", "windows", "resource", "performance", "ram",
        "memory", "cpu", "processor", "disk", "storage", "tell", "give", "list", "display",
        "running", "kill", "terminate", "close", "stop", "quit", "end", "force", "read", "clipboard",
    }
    return (prefix in natural_prefixes or prefix in _KNOWN_APP_NAMES) and _try_os_intent(text) is not None
