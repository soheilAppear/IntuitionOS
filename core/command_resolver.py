...
Providers describe commands in the execution shell. They never run candidates.
Scores are ranking weights, not probabilities. Feedback stores command tokens
only; argument values and full command lines remain transient in the session.
"""
...
KNOWN_COMMANDS = [
    ...
    '/config', '/reload', '/tasks', '/done', '/delete', '/snooze', '/hw',
    '/task_payload', '/safe', '/exec', '/write', '/read', '/undo', '/journal',
    '/capabilities', '/forget', '/episodes', '/calibration', '/thresholds', '/rules',
]
...
def _project(context):
    ...
    return hashlib.sha256(os.path.normcase(os.path.abspath(cwd)).encode()).hexdigest() if cwd else ''
...
class StaticCommandProvider:
    ...
    def __init__(self, names, namespace='shell', case_sensitive=True, kind='executable'):
        self.entries = tuple(n if isinstance(n, Command) else Command(n, namespace, kind, case_sensitive)
                             for n in names)
...
class IntuitionCommandProvider(StaticCommandProvider):
    def __init__(self):
        super().__init__(KNOWN_COMMANDS + ['ls', 'tree'], namespace='intuitionos', case_sensitive=True, kind='internal')


class ShellBuiltinProvider(StaticCommandProvider):
    CMD = 'assoc break call cd chdir cls color copy date del dir echo endlocal erase exit for ftype goto if md mkdir mklink move path pause popd prompt pushd rd rem ren rename rmdir set setlocal shift start time title type ver verify vol'.split()
    SH = ': . [ alias bg break cd command continue eval exec exit export false fc fg getopts hash jobs kill printf pwd read readonly return set shift test times trap true type ulimit umask unalias unset wait'.split()

    def __init__(self, shell='sh'):
        super().__init__(self.CMD if shell == 'cmd' else self.SH if shell == 'sh' else [],
                         case_sensitive=shell == 'sh', kind='builtin')
        ...
        if shell == supported_shell:
            for item in commands:
                if isinstance(item, Command):
                    ...
                    entries.append(Command(item['name'], 'shell', item.get('kind', 'command'), shell == 'sh'))
                    source = item.get('source')
                    if shell in {'powershell', 'pwsh'} and source and item.get('kind', '').lower() in {'alias', 'function', 'cmdlet'}:
                        entries.append(Command(source + '\\' + item['name'], 'shell', item.get('kind', 'command'), False))
...
class PathExecutableProvider:
    ...
    def __init__(self, *, shell='sh', environ=None, ttl=3.0):
        self.shell, self.environ, self.ttl = shell, os.environ if environ is None else environ, ttl
                ...
                with os.scandir(directory) as items:
                    for item in items:
                        try:
                            ...
                            for name in names:
                                key_name = name.casefold() if self.shell != 'sh' else name
                                found.setdefault(key_name, Command(name, case_sensitive=self.shell == 'sh', kind='script' if suffix in {'.cmd', '.bat', '.ps1'} else 'executable'))
                        except OSError:
                            continue
            except OSError:
                continue
...
class GitSubcommandProvider:
    ...
    """
    DEFAULT = 'add am annotate apply archive bisect blame branch bundle cat-file check-attr check-ignore check-mailmap check-ref-format checkout cherry cherry-pick clean clone column commit config count-objects credential describe diff difftool fast-export fast-import fetch filter-branch for-each-ref format-patch fsck gc grep help init log ls-files ls-remote ls-tree maintenance merge merge-base mergetool mv notes pack-objects prune pull push range-diff read-tree rebase reflog remote repack replace request-pull reset restore revert rev-list rev-parse rm shortlog show show-branch sparse-checkout stash status submodule switch symbolic-ref tag update-index update-ref verify-commit verify-tag version whatchanged worktree write-tree'.split()
    ...
    def commands(self, context=None):
        ...
        configs = [Path.home() / '.gitconfig', Path.home() / '.config' / 'git' / 'config']
...
class InstalledGitSubcommandProvider:
    ...
    def commands(self, context=None):
        ...
        if not executable:
            self.available = False
            return ()
        key = (executable, cwd, os.environ.get('GIT_CONFIG_GLOBAL'), os.environ.get('GIT_CONFIG_SYSTEM'), os.environ.get('GIT_DIR'))
        now = time.monotonic()
        ...
        try:
            result = subprocess.run([executable, '--list-cmds=main,others,alias'], cwd=cwd,
                                    capture_output=True, text=True, timeout=1.0,
                                    creationflags=getattr(subprocess, 'CREATE_NO_WINDOW', 0))
            if result.returncode == 0:
                names = tuple(Command(name, 'git', 'subcommand', True) for name in sorted(set(result.stdout.split()))
...
def spelling_distance(left: str, right: str) -> int:
    ...
    for i, a in enumerate(left, 1):
        ...
        for j, b in enumerate(right, 1):
            row[j] = min(row[j-1] + 1, rows[i-1][j] + 1, rows[i-1][j-1] + (a != b))
...
def _tokens(text, shell):
    """Return safe spans; reject syntax whose interpretation is shell-dependent."""
    ...
    for i, char in enumerate(text):
        ...
        if quote:
            ...
            elif (quote == '"' and (char in '$`' or (char == '%' and shell == 'cmd'))) or (char == '\\' and shell == 'sh'):
                return None
            continue
        if char == '"' or (char == "'" and shell != 'cmd'):
            if start is None:
                start = i
            quote = char
        elif char in '|&;<>`(){}$!^' or (char == '%' and shell == 'cmd') or (char == '\\' and shell == 'sh'):
            return None
        elif char.isspace():
...
class CommandResolver:
    def __init__(self, providers=None, *, shell='sh', feedback=None, git_provider=None, max_candidates=3):
        self.shell = shell
        self.providers = list(providers if providers is not None else [IntuitionCommandProvider(), ShellBuiltinProvider(shell), PathExecutableProvider(shell=shell)])
    ...
    def resolve(self, text, context=None):
        ...
        if not token.startswith('/') and is_app_command(original):
            return Resolution(original, 'exact', reason='Existing IntuitionOS intent; input is preserved.', span=(start, end), namespace='intuitionos')
        if token in {'ls', 'tree'}:
            return Resolution(original, 'exact', reason='IntuitionOS command; input is preserved.', span=(start, end), namespace='intuitionos')
        ...
        if token.startswith('/') and not (self.shell == 'sh' and os.path.isfile(token)):
            if token != '/exec':
                commands = [c for p in self.providers for c in p.commands(context) if c.namespace == 'intuitionos']
                return self._rank(original, (start, end), commands, 'intuitionos', context)
            body = text[end:]
            if not body.strip():
                return Resolution(original, 'incomplete', reason='Enter a shell command after /exec.', namespace='shell')
        ...
        if spans is None:
            return Resolution(original, 'unsupported', reason='Shell operators, expansions, escapes or unmatched quotes prevent a safe correction span; input is unchanged.', namespace='shell')
        span = spans[0]
        head = original[span[0]:span[1]]
        if any(c in head for c in "'\"="):
            return Resolution(original, 'unsupported', reason='Quoted command names and environment assignments are not corrected.', span=span, namespace='shell')
        commands = [c for p in self.providers for c in p.commands(context) if c.namespace != 'intuitionos']
        exact = next((c for c in commands if self._equal(head, c)), None)
        if '/' in head or '\\' in head:
            if exact:
                return Resolution(original, 'exact', reason='Available qualified command; input is preserved.', span=span, namespace='shell')
            if self.shell in {'powershell', 'pwsh'} and re.fullmatch(r'[A-Za-z0-9_.-]+\\[^\\/]+', head):
                module = head.split('\\', 1)[0].casefold() + '\\'
                qualified = [c for c in commands if c.name.casefold().startswith(module) and c.kind.lower() in {'alias', 'function', 'cmdlet'}]
                if qualified:
                    return self._rank(original, span, qualified, 'shell', context)
            path = Path(head)
            if not path.is_absolute():
                path = Path(_value(context, 'cwd', None) or os.getcwd()) / path
            extensions = {e.lower() for e in os.environ.get('PATHEXT', '.COM;.EXE;.BAT;.CMD').split(';')}
            if self.shell in {'powershell', 'pwsh'}:
                extensions.add('.ps1')
            available = path.is_file() and (os.access(path, os.X_OK) if self.shell == 'sh' else path.suffix.lower() in extensions)
            return Resolution(original, 'exact' if available else 'unsupported', reason='Explicit executable path is preserved.' if available else 'Executable path is unavailable; paths are not corrected.', span=span, namespace='shell')
        if exact and exact.kind.lower() in {'executable', 'application'} and head.casefold() in {'git', 'git.exe'} and len(spans) > 1:
            subspan = spans[1]
            subcommand = original[subspan[0]:subspan[1]]
            if subcommand.startswith('-'):
                return Resolution(original, 'exact', reason='Known git command; options and arguments remain untouched.', span=span, namespace='shell')
            if any(c in subcommand for c in "'\"/\\="):
                return Resolution(original, 'unsupported', reason='Git subcommand grammar is not established.', span=subspan, namespace='git')
            git_commands = list(self.git_provider.commands(context))
            if isinstance(self.git_provider, InstalledGitSubcommandProvider) and not self.git_provider.available:
                return Resolution(original, 'exact', reason='Git is available; subcommand metadata is unavailable, so arguments remain untouched.', span=span, namespace='shell')
            git_commands.extend(Command(c.name[4:], 'git', 'external-subcommand', True) for c in commands if c.name.startswith('git-'))
    ...
    def _equal(token, command):
        return token == command.name if command.case_sensitive else token.casefold() == command.name.casefold()
    ...
    def _rank(self, original, span, commands, namespace, context):
        ...
        if any(self._equal(token, c) for c in commands):
            return Resolution(original, 'exact', reason='Available command; input is preserved.', span=span, namespace=namespace)
        if len(token.lstrip('/')) < 2:
            return Resolution(original, 'incomplete' if token == '/' else 'unsupported', reason='Too little command text for a safe suggestion.', span=span, namespace=namespace if namespace != 'shell' else 'unknown')
        candidates, seen = [], set()
        for command in commands:
            ...
            if not prefix and (distance > max_distance or similarity < 0.55):
                continue
            baseline = (0.68 + 0.22 * len(typed) / len(wanted)) if prefix else similarity
            boost = self.feedback.boost(token, name, namespace, context) if self.feedback else 0.0
            score = round(baseline + boost, 6)
            reason = 'Complete command name' if prefix else f'Spelling distance {distance} (adjacent transpositions count as one)'
            if boost:
                reason += '; prior explicit acceptance and project context'
            candidates.append(Candidate(original[:span[0]] + name + original[span[1]:], name, namespace, score, reason, span))
        candidates.sort(key=lambda c: (-c.score, c.token.casefold(), c.token))
        if not candidates:
            return Resolution(original, 'unsupported', reason='No plausible available command; input is unchanged.', span=span, namespace=namespace if namespace != 'shell' else 'unknown')
        status = 'incomplete' if candidates[0].reason.startswith('Complete') else 'correction'
        if len(candidates) > 1 and candidates[0].score - candidates[1].score < 0.045:
            status = 'ambiguous'
        return Resolution(original, status, candidates[:self.max_candidates], 'Choose a displayed suggestion or keep the original.', span, namespace)
...
class CorrectionFeedbackStore:
    ...
    @property
    def generation(self):
        rows = self.mem.query('SELECT generation FROM command_correction_meta WHERE id=1')
    ...
    def record_display(self, resolution, context=None, ts=None):
        ...
        candidates = [{'token': _safe_token(c.token), 'namespace': c.namespace, 'score': c.score} for c in resolution.candidates]
        return self.mem.insert('INSERT INTO command_corrections(ts,namespace,original_token,candidates_json,project_key) VALUES(?,?,?,?,?)',
                               (time.time() if ts is None else ts, resolution.namespace, token, json.dumps(candidates), _project(context)))

    def record_selection(self, event_id, candidate_index=None, manual_token=None, ts=None):
        if not self.active or event_id is None:
            return
        rows = self.mem.query('SELECT candidates_json FROM command_corrections WHERE id=?', (event_id,))
        if not rows:
            return
        choices = json.loads(rows[0][0])
        if candidate_index is not None and (isinstance(candidate_index, bool) or not isinstance(candidate_index, int) or not 0 <= candidate_index < len(choices)):
            raise ValueError('Invalid correction selection')
        selected = choices[candidate_index]['token'] if candidate_index is not None else None
        self.mem.execute('UPDATE command_corrections SET selected_token=?,selected_ts=?,accepted=?,manual_token=? WHERE id=?',
                         (selected, time.time() if ts is None else ts, 1 if selected else None, _safe_token(manual_token), event_id))
    ...
    def record_outcome(self, event_id, outcome, ts=None):
        if self.active and event_id is not None:
            ...
            category = outcome if outcome in {'ok', 'error', 'denied', 'cancelled', 'pending', 'undone', 'unsupported'} else 'unknown'
            self.mem.execute('UPDATE command_corrections SET outcome=?,outcome_ts=? WHERE id=?',
    ...
    def boost(self, original, candidate, namespace, context=None):
        ...
        rows = self.mem.query('SELECT original_token,project_key,selected_ts FROM command_corrections WHERE namespace=? AND selected_token=? AND accepted=1 AND ts<=? AND selected_ts<=? ORDER BY selected_ts DESC LIMIT 256',
        ...
        return min(0.35, 0.045 * math.log1p(len(rows)) + 0.09 * math.log1p(same) + 0.08 * math.log1p(local) + 0.035 * recency)

    def forget(self, before=None, *, since=None):
        """Forget all history, or rows older than the episode cutoff `before`.

        `since` remains a temporary compatibility spelling for that cutoff.
        """
        ...
        self.mem.execute('UPDATE command_correction_meta SET generation=generation+1 WHERE id=1')


class CorrectionSession:
    """A displayed revision is a one-use commitment, never an execution permit."""
    ...
    def update(self, text, context=None):
        ...
        if self.resolution is not None and self.resolution.original == text and self.token and generation == self._generation:
            return self.resolution
        previous = self.resolution
        resolution = self.resolver.resolve(text, context)
        if previous and previous.original != text and self.resolver.feedback and not self._committed:
            ...
            self.resolver.feedback.record_selection(self.feedback_id, manual_token=manual)
        self.revision += 1
        self.token = uuid.uuid4().hex
        self.resolution = resolution
        self._context, self._generation, self._committed = context, generation, False
        self.feedback_id = self.resolver.feedback.record_display(self.resolution, context) if self.resolver.feedback else None
        return self.resolution

    def snapshot(self):
        data = self.resolution.to_dict() if self.resolution else Resolution('', 'incomplete').to_dict()
        data.update(token=self.token, revision=self.revision)
        return data

    def commit(self, original, *, token, revision, candidate_index=None):
        if (not self.token or self._committed or token != self.token or revision != self.revision or
                self.resolution is None or original != self.resolution.original or self._generation != self.resolver.generation):
            raise ValueError('Correction is stale; display the current input again.')
        candidates = self.resolution.candidates
        if candidate_index is not None and (isinstance(candidate_index, bool) or not isinstance(candidate_index, int) or not 0 <= candidate_index < len(candidates)):
            raise ValueError('Invalid correction selection')
        selected = original if candidate_index is None else candidates[candidate_index].text
...
def create_default_resolver(memory=None, enabled=True, shell=None):
    ...
    if shell in {'powershell', 'pwsh'}:
        ...
        providers.append(EnvironmentCatalogProvider(discover_powershell_commands(shell), shell=shell, supported_shell=shell))
    ...
    return CommandResolver(providers, shell=shell, feedback=feedback, git_provider=git_provider)
...
def learning_text(text):
    ...
    if head.casefold() in {'git', 'git.exe'} and len(parts) > 1 and parts[1] in GitSubcommandProvider.DEFAULT:
        return head + ' ' + parts[1]
    return head
