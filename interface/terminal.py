# Terminal UI with visible shared command resolution and gated actions.

import os, re, yaml, json, time, threading
...
from core.command_resolver import (
    CorrectionSession, create_default_resolver, learning_text, legacy_fuzzy_slash,
)
...
class CorrectionPrompt:
    ...
    def __init__(self, resolver, context_fn=None, **session_kwargs):
        ...
        def submit(event):
            ...
            if not raw.strip():
                ...
                return
            if (self._snapshot is None or self._displayed_text != raw
                    or self._selected_index != self._displayed_index):
                event.app.invalidate()
                return
            try:
                selected = self.corrections.commit(
                    raw, token=self._snapshot['token'],
        ...
        self.session = PromptSession(
            'io> ', key_bindings=bindings, bottom_toolbar=self._toolbar,
    ...
    def _toolbar(self):
        ...
        if not raw.strip():
            return [('class:correction.help', 'Command corrections appear here while you type.')]
        if not self._submitted and (self._snapshot is None or self._displayed_text != raw):
            self.corrections.update(raw, self.context)
            self._snapshot = self.corrections.snapshot()
            self._displayed_text = raw
            self._selected_index = (
                0 if self._snapshot['candidates'] and self._keep_original_text != raw else None
            )
        if self._snapshot is None:
            return []
        candidates = self._snapshot['candidates']
        chosen = candidates[self._selected_index] if self._selected_index is not None else None
        ...
        if candidates:
            ...
            for index, candidate in enumerate(candidates):
                style = 'class:correction.selected' if self._selected_index == index else ''
...
def main():
    ...
    while True:
        ...
        if resolved.namespace in ('shell', 'git') or re.match(r'^/exec\s', user):
            ...
            _signals.update(keystroke_prefix=learned_input, predicted=None,
                            predicted_conf=None, accepted_prediction=None)
        ...
        if episodes.enabled:
            predictor.update(Episode(ts=time.time(), action=learned_input, context=_ctx,
        ...
        try:
            ...
            if user.startswith('/done '):
                try:
                    tid=int(user.split(' ',1)[1]); rprint(dispatch('complete_task', task_id=tid))
                except Exception: rprint('usage: /done <id>')
                continue
            if user.startswith('/delete '):
                try:
                    tid=int(user.split(' ',1)[1]); rprint(dispatch('delete_task', task_id=tid))
            ...
            if user.startswith('/snooze '):
                ...
                continue
            if user=='/hw':
                rprint(dispatch('hw_list')); continue
            if user.startswith('/hw schema '):
                name=user.split(' ',2)[2]; rprint(dispatch('hw_schema', device=name)); continue
            if user.startswith('/task_payload '):
                try:
                    _, payload_json, when = user.split(' ', 2)
                    payload=json.loads(payload_json.strip('\"\''))
                    rprint(dispatch('create_task', text=None, when=when, repeat='', payload=payload))
                except Exception as e:
            ...
            if user=='/forget':
                ...
                predictor = Predictor(
                    ...
                    half_life_s=float(pcfg.get('half_life_s', 7*24*3600)),
                    min_episodes=int(pcfg.get('min_episodes', 50)),
                    calibrator=calib_store.load(), rules=rules,
                )
                sensor = ContextSensor(journal=get_journal())
                window.reset()
                resolver = create_default_resolver(mem, enabled=lambda: episodes.enabled)
                ...
                ant = make_anticipator(brain, cfg, predictor=predictor, sensor=sensor)
                _ant_ref[0] = ant
                rprint(f'Forgot {forgotten} episode(s) and learned correction preferences.')
                continue
            if user=='/episodes':
            ...
            if re.match(r'^/exec\s', user):
                try:
                    ...
                    if legacy.startswith('"'):
                        idx=legacy.find('"',1)
                        if idx!=-1:
                            cmd=legacy[1:idx]; tail=legacy[idx+1:].strip(); cwd=tail or '.'
                    if cmd is not None:
                        rprint(dispatch('run_local', cmd=cmd, cwd=cwd))
                    elif resolved.status == 'unsupported' and resolved.namespace in ('shell', 'git'):
            ...
            if user.startswith('/read '):
                try:
                    path=user.split(' ',1)[1].strip(); rprint(dispatch('read_file', path=path))
                except Exception as e:
            ...
            if user=='ls':
                rprint(dispatch('list_dir', path='.')); continue
            if user=='tree':
                rprint(dispatch('list_tree', path='.')); continue
            ...
            if resolved.namespace in ('shell', 'git'):
                if resolved.status == 'unsupported':
                    ...
                    rprint({'error': 'Unknown command kept unchanged. Select a displayed correction or edit it.'})
                    _outcome['value'] = 'unsupported'
                continue
            if user.startswith('/'):
                rprint({'error': 'Unknown IntuitionOS command kept unchanged. Type /help for commands.'})
                _outcome['value'] = 'unsupported'
                continue
            ...
            if _m:
                rprint(dispatch('create_task', text=_m.group(1).strip(), when=_m.group(2).strip()))
                continue
            ...
            while out.get('needs_confirmation'):
                ...
                rprint(Panel.fit(escape(f"{out['capability']}  {detail}" + chr(10) + f"{out.get('reason','')}"),
                                 title=title,
